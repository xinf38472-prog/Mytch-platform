...
本项目没有让学生直接输入技能分或软技能分。学生端只收集 GPA、实习经历、技能证书、项目经历和行为描述。后端通过规则化评分函数识别 GPA、实习时长、技能关键词、证书、项目经历、责任心、抗压、沟通协作等信息，并自动生成五维能力指标。这些指标写入 MySQL 的 `workers` 表，用于效用矩阵和稳定匹配；原始文字材料保存在 `worker_profiles` 表中，用于展示、解释和审计。


## 重要说明：学生端输入逻辑

本版本采用“文字资料输入 + 后端规则评分”的逻辑：

- 学生端不再让学生直接填写技能分、实习分、软技能分。
- 学生只填写 GPA/学历、实习项目、技能证书、个性和软技能的文字资料。
- 页面中的五维能力分是系统根据 MySQL 中 `worker_profiles` 的文字资料计算后写入 `workers` 表的结果。
- 如果仍看到左侧可编辑的数字输入框，请确认浏览器没有缓存旧版 `app.js`，可按 `Command + Shift + R` 强制刷新，或重新解压本项目包。
